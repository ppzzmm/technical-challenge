class Api::V1::RepositoriesController < ApplicationController
  def index
    ...
    repos = conn.get("https://api.github.com/user/repos?user=#{user_params}", {per_page: 100, sort: 'updated'}).body
    db_user = User.all.find { |u| u.github_id == user['id'] }
    if db_user.nil?
      db_user = User.create({github_id: user['id'], login: user['login'], url: user['html_url'], name: user['name'], email: user['email'], avatar_url: user['avatar_url'], repositories: repos})
    end
